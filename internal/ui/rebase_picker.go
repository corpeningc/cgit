...
func NewRebasePickerModel(repo *git.GitRepo, entries []git.RebaseEntry) RebasePickerModel {
	return RebasePickerModel{
		...
		actionStyles: map[string]lipgloss.Style{
			"pick":   lipgloss.NewStyle().Foreground(colorGreen),
			"reword": lipgloss.NewStyle().Foreground(colorCyan),
			"edit":   lipgloss.NewStyle().Foreground(colorOrange),
			"squash": lipgloss.NewStyle().Foreground(colorPink),
			"fixup":  lipgloss.NewStyle().Foreground(colorGray),
			"drop":   lipgloss.NewStyle().Foreground(colorRed),
		},
	}
}
