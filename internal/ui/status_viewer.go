...
func (m StatusViewerModel) View() string {
	...
	// Tab headers
	stagedTab := m.inactiveTabStyle.Render(fmt.Sprintf("  Staged (%d)  ", len(m.stagedFiles)))
	unstagedTab := m.inactiveTabStyle.Render(fmt.Sprintf("  Unstaged (%d)  ", len(m.unstagedFiles)))
	if m.currentTab == 0 {
		stagedTab = m.activeTabStyle.Render(fmt.Sprintf("  Staged (%d)  ", len(m.stagedFiles)))
	} else {
		unstagedTab = m.activeTabStyle.Render(fmt.Sprintf("  Unstaged (%d)  ", len(m.unstagedFiles)))
	}
	sections = append(sections, lipgloss.JoinHorizontal(lipgloss.Top, stagedTab, unstagedTab))
	sections = append(sections, "")
...
func StartStatusViewer(repo *git.GitRepo) error {
	for {
		...
		// Launch filepicker, then loop back to status viewer
